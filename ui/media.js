      ...
      async function selectVideo() {
        try {
          ...
          if (raw && raw.indexOf('/') !== -1) {
            ...
            const size = await (async function(){
              try {
                const j = await evalExtendScript('AEFT_statFile', { path: raw });
                if (j && j.ok && typeof j.size === 'number') return j.size;
              } catch(_){ }
              return await new Promise(resolve=>{ const safe = String(raw).replace(/\\/g,'\\\\').replace(/\"/g,'\\\"'); const es = `(function(){try{var f=new File("${safe}");if(f&&f.exists){return String(f.length||0);}return '0';}catch(e){return '0';})()`; cs.evalScript(es, function(r){ var n=Number(r||0); resolve(isNaN(n)?0:n); }); });
            })();
